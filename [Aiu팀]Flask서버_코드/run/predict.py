...
def predict_task():
    ...
    ############ 샘플링 (60개) ############
    task_status["current_step"] = "[predict] Sampling 60 data points"
    print(task_status["current_step"])
    ...
    print("[predict] Final 'time' columns created successfully.")

    ############ 데이터 처리 - 열이름 변경 ############
    task_status["current_step"] = "[predict] Renaming columns"
    print(task_status["current_step"])
    ...
    ############ 앞의 60개 행 삭제 ############
    task_status["current_step"] = "[predict] Deleting the first 60 rows"
    print(task_status["current_step"])
    ...
    print("[predict] Deleted the first 60 rows from the 'predict' table.")
    ...
    # INSERT 쿼리 생성
    insert_query = f"INSERT INTO predict ({columns}) VALUES ({placeholders})"
    ...
    task_status["current_step"] = "[predict] Inserting new data into MySQL"
    print(task_status["current_step"])
    ...
    return {"message": "Autoencoder task completed!"}